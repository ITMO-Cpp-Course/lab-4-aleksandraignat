...
// Тест проверяет RAII поведение FileHandle: файл автоматически открывается в конструкторе, файл автоматически закрывается в деструкторе, данные успешно записываются и читаются
...
// Тест проверяет обработку ошибок: открытие несуществующего файла, чтение из закрытого файла, запись в закрытый файл, повторное открытие уже открытого файла
...
// Тест проверяет: при перемещении владение ресурсом передаётся новому объекту, исходный объект становится невалидным
...
// Тест проверяет кеширование: повторный запрос того же файла возвращает тот же shared_ptr, счётчик ссылок корректно увеличивается
...
// Тест проверяет принудительное удаление из кеша: после evict() новый запрос возвращает другой объект
...
// Тест проверяет потокобезопасность ResourceManager: несколько потоков одновременно запрашивают один и тот же файл, все должны получить один и тот же shared_ptr
