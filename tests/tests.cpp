...
// Тест проверяет RAII поведение FileHandle: файл автоматически открывается в конструкторе, файл автоматически
// закрывается в деструкторе, данные успешно записываются и читаются
...
// Тест проверяет обработку ошибок: открытие несуществующего файла, чтение из закрытого файла, запись в закрытый файл,
...
// Тест проверяет кеширование: повторный запрос того же файла возвращает тот же shared_ptr, счётчик ссылок корректно
// увеличивается
...
// Тест проверяет потокобезопасность ResourceManager: несколько потоков одновременно запрашивают один и тотже файл, все
// должны получить один и тот же shared_ptr
